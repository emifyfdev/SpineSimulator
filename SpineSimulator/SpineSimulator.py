...
class SpineSimulatorV3:
    ...
    def __init__(self):
        ...
        self._interaction_handle_ref_pos = None
    ...
    def _joint_matrix_np(self, label, rotation_scale=1.0):
        ...
        # La traslación también se atenúa por rotation_scale: si no, una vértebra
        # intermedia de la cadena (k != idx) filtra su traslación completa en
        # CADA vértebra que la incluya en su cadena, aunque su rotación esté
        # reducida por la "curvatura local". Eso se veía como un desplazamiento
        # inesperado al rotar una vértebra distinta a la que tiene la traslación.
        return T([tx * rotation_scale, ty * rotation_scale, tz * rotation_scale]) @ T(pivot) @ R @ T(-pivot)
    ...
    def _sync_interaction_handle_to_active(self):
        ...
        self._interaction_handle_ref_pos = np.array(handle_pos, dtype=float)
    ...
    def _on_interaction_handle_modified(self, caller, event=None):
        ...
        try:
            ...
            # Traslación: delta desde la posición de referencia del handle guardada al sincronizar
            if not self.native_rotation_only:
                ref_pos = self._interaction_handle_ref_pos
                if ref_pos is None:
                    ref_pos = self._interaction_handle_display_position(self._motion_pivot_world(label))
